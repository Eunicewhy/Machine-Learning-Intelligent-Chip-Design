...
Vector3D Input(int height, int width, int channels, string file_name){
    ...
    fin.close();
    // Zero Padding�G�W�B���U�� 2 �C 0 �A�U�B�k�U�� 1 �C 0
...
Vector3D Convolution(int num_filters, int filter_size, int stride_conv, int channels, int padding_conv, Vector3D padded_image, string conv_weight_file, string conv_bias_file){
    ...
    // �s filter ��T (kernels * channels * filter_size * filter_size)
                   ...
                   for (int c = 0; c < channels; c++){
                        for (int ki = 0; ki < filter_size; ki++){
                             for (int kj = 0; kj < filter_size; kj++){
                              ...
                              //     if(f == num_filters - 1) cout << conv_filters[f][c][ki][kj] << " ";
                             }
                        }
                   }
                   // �p�� bias
...
Vector1D Fully_Connect(int input_size, int num_neurons, string fc_weight_file, string fc_bias_file, Vector1D fc_input){
    ...
    for (int n = 0; n < num_neurons; n++) {
        ...
        sum += fc_biases[n];
        if(fc_weight_file == "./data/fc8_weight.txt") fc_output[n] = sum;  // �Y�{�b�� fc8 �A ���ϥ� ReLU
...
void AlexNet::run() {
    ...
    Vector1D flat = Flatten(256, 6, 6, pool_output5);
    Vector1D result = fc_output8;
    
    // Softmax Layer
    Vector1D softmax_output = Soft_Max(result);
//     sort(result.begin(), result.end(), greater<float>());
//     sort(softmax_output.begin(), softmax_output.end(), greater<float>());

     // �N�̲׵��G��X�� output port
     for(int i = 0; i < 1000; i++){
          softmax_output_result[i].write(softmax_output[i]);
          fc_output_result[i].write(result[i]);
     }
    
}

// Monitor �Ҳժ� monitor_output()�G�L�X Top 100 ���G
