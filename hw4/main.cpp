...
int sc_main(int argc, char* argv[]) {
    ...
    // Controller / ROM connection
    Controller* controller = new Controller("controller");
    ROM* rom = new ROM("rom");

    sc_signal<int> layer_id_sig;
    sc_signal<bool> layer_type_sig, layer_valid_sig;
    sc_signal<float> rom_data;
    sc_signal<bool> rom_data_valid;

    controller->clk(clk);
    controller->rst(rst);
    controller->layer_id(layer_id_sig);
    controller->layer_id_type(layer_type_sig);
    controller->layer_id_valid(layer_valid_sig);
    controller->data(rom_data);
    controller->data_valid(rom_data_valid);

    rom->clk(clk);
    rom->rst(rst);
    rom->layer_id(layer_id_sig);
    rom->layer_id_type(layer_type_sig);
    rom->layer_id_valid(layer_valid_sig);
    rom->data(rom_data);
    ...
    cout << "Simulation start at " << sc_time_stamp() << endl;
    sc_start();
